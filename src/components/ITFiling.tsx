...
import ITFilingHistoryTable from "./ITFilingHistoryTable";
...
export default function ITFiling() {
  ...
  const [records, setRecords] = useState<ITFilingRecord[]>([]);
  ...
  async function refreshHistory() {
    ...
    try {
      const res = await fetch("/api/it-filing");
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="space-y-6">
          ...
          <button
            ...
            onClick={async () => {
              ...
              setSaveSuccess(null);
              try {
                const res = await fetch("/api/it-filing/save", {
                  method: "POST",
                ...
                const json = await res.json();
                if (!res.ok) throw new Error(json?.error ?? "Failed to save IT filing");
                const record = json.record as ITFilingRecord;
                ...
                setSaveError(e?.message ?? "Failed to save IT filing");
              } finally {
                setSaving(false);
              }
            }}
      ...
      <div>
        {historyLoading ? (
          <div className="text-sm text-gray-500">Loading saved IT filings...</div>
        ) : historyError ? (
          <div className="text-red-400 text-sm">{historyError}</div>
        ) : (
          <ITFilingHistoryTable
            records={records}
