...
import JewelLoanHistoryTable from "./JewelLoanHistoryTable";
...
export default function JewelLoan() {
  ...
  const [records, setRecords] = useState<JewelLoanRecord[]>([]);
  ...
  async function refreshHistory() {
    ...
    try {
      const res = await fetch("/api/jewel-loan");
        ...
        <div className="space-y-6">
          ...
          <button
            ...
            onClick={async () => {
              ...
              setSaveSuccess(null);
              try {
                const res = await fetch("/api/jewel-loan/save", {
                  method: "POST",
                ...
                const json = await res.json();
                if (!res.ok)
                  throw new Error(json?.error ?? "Failed to save jewel loan");
                const record = json.record as JewelLoanRecord;
                setSelectedId(record.id);
                setRecords((prev) => [record, ...prev]);
                setWeight(record.weight);
                setPurity(record.purity);
                setMarketRate(record.marketRate);
                setLoanLTV(record.loanLTV);
                setInterestRate(record.interestRate);
                setSaveSuccess("Saved to Google Sheets.");
              } catch (e: any) {
                setSaveError(e?.message ?? "Failed to save jewel loan");
              } finally {
                setSaving(false);
              }
            }}
          ...
          {saveError ? <div className="text-red-400 text-sm">{saveError}</div> : null}
          {saveSuccess ? <div className="text-green-400 text-sm">{saveSuccess}</div> : null}
        </div>
      </div>

      <div>
        {historyLoading ? (
          <div className="text-sm text-gray-500">Loading saved jewel loans...</div>
        ) : historyError ? (
          <div className="text-red-400 text-sm">{historyError}</div>
        ) : (
          <JewelLoanHistoryTable
            records={records}
